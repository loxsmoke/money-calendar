...
public static class UpdateService
{
        ...
        Version current, HttpClient? http = null, CancellationToken ct = default)
    {
        try
        {
            ...
            var (url, size) = FindPortableAsset(root, version);
    ...
    /// <summary>The portable zip for that version, when the release carries one.</summary>
    private static (string? Url, long Size) FindPortableAsset(JsonElement root, string version)
    {
        if (!root.TryGetProperty("assets", out var assets) || assets.ValueKind != JsonValueKind.Array)
            return (null, 0);

        var wanted = UpdateCheck.PortableAssetName(version);
